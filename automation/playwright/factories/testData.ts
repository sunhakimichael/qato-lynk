...
/**
 * Always CIMB Niaga VA — the checkout selects it by code in
 * PublicCheckoutPage.selectCimbNiagaVirtualAccount(). These fields describe
 * how that method is labelled downstream.
 */
...
export function getTestPaymentMethod(): TestPaymentMethod | undefined {
  const config = parseTestDataConfig();
  const { TEST_PAYMENT_METHOD_CHANNEL_LABEL, TEST_PAYMENT_METHOD_DISPLAY_NAME } = config;

  if (TEST_PAYMENT_METHOD_CHANNEL_LABEL === undefined || TEST_PAYMENT_METHOD_DISPLAY_NAME === undefined) {
    return undefined;
  }
