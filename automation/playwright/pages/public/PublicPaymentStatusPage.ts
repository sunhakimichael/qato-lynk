...
export class PublicPaymentStatusPage {
   ...
   *
   * Generating the VA can take a long time, so both the redirect and the
   * invoice section get the full PAGE_LOAD_TIMEOUT_MS. No reload here:
   * this page is reached right after submitting the purchase.
   */
  async waitForLoad(): Promise<void> {
    ...
    await waitForPageReady(this.page, publicPaymentStatusLocators.invoiceSection(this.page), {
      pageName: "Payment",
    });
  }
