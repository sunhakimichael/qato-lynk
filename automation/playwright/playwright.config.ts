...
export default defineConfig({
  ...
  expect: { timeout: env.DEFAULT_ACTION_TIMEOUT_MS },
  fullyParallel: true,
  ...
  projects: [
    ...
    },
    {
      ...
      // Member login uses the MEMBER_EMAIL that checkout just bought with
      // (randomized per run), so the storefront checkout must finish first.
      dependencies: ["public"],
